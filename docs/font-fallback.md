...
本分析以实际回归用的 libass 0.17.5 为准；缺字回退尚未在 assfonts-rs 中实现，当前实际缺字仍报错。2026-09-08 的同分候选修复只对齐载入顺序，不宣称完整字体选择算法已经一致。
...
如果完整字体基准本身找不到字形，应区分两种目标：忠实复现原来的 .notdef，或指定新字体让它可读。后者是视觉行为变更，不是“无损兼容”。建议保留严格缺字模式，并另设明确的 libass-compatible 策略，报告每次回退与仍缺失的字符。
