...
Rust 的 `AssCodec` 和 `ConfiguredAssCodec::default()`、CLI 和新建 WASM engine 均默认 `strict`。
显式兼容选项分别为 `AssCodec::with_mode(ParseMode::Compatible)`（实现 `SubtitleCodec`）、
CLI `--parse-mode compatible`、WASM `af_set_parse_mode(engine, 1)`；`0` 切回严格模式。
测试/参考宿主对应 `MemoryEngine.setParseMode('compatible')`。
处理报告及 CLI check 报告增加 `parse_mode` 字段，保持 schema_version 1（新增字段）；
严格失败仍使用现有错误响应，不生成处理结果。旧版宿主若依赖宽松输入，需显式设置兼容模式。

严格模式以兼容修复前的标签与数值检查为基线，但不恢复旧版字体分析：

- 使用完整标签名检查，拒绝未知标签，包括疑似拼写错误 `\blu0.8`、`\bklur0.5`，不会自动改成 `\blur`。
- 对 `\b`、`\i`、`\p`、`\q` 的非空参数要求完整 i32 整数，拒绝小数、尾随文本、括号包装及溢出；
  `\i` 只接受 0/1，`\q` 只接受 0–3。空参数保持原有重置/默认规则。
  `\b-1`、`\b2`、`\b20` 等低字重回退继续允许，不把低字重修复撤销。
- 递归检查嵌套动画内的标签；允许已支持的嵌套视觉动画、无内部标签的动画和已支持的缺右括号恢复。
  字体/状态变化限制与深度上限仍由当前分析器执行。检查器不使用旧版平衡括号结果推导字体状态。
- 只允许真正的空反斜杠标签恢复；未知 Unicode 标签、反斜杠后空白等旧版拒绝形式仍拒绝。
- 这是有限的接受规则，不是完整 ASS 规范检查器；视觉标签的所有数值、动画时间参数尚未全面校验。
  字体名/重置样式名允许文本，颜色与浮点视觉参数不会被一概当作整数检查。

错误包含 Dialogue 序号、该条 Text 内的零基 UTF-8 字节偏移和标签/原因；不是全文件行列位置。
严格检查通过后，两种模式统一走当前字体分析和嵌入逻辑，不修改字幕正文。
语法模式不改变 `missing_glyph_policy`（仍默认 warn），无法安全分析的输入两种模式都拒绝。
...
- ass-core 继续负责段结构、样式和事件字段解析，依赖仍固定在上述 fork。
  ...
  严格模式单独复用通用标签解析器做接受检查，不用它生成字体需求。
...
随后在兼容接受规则下，42 份覆盖标签与 17 份动画问题全部解决，累计 227/229，剩余两份缺字体；
详见 [覆盖标签与嵌套动画验证](validation/2026-09-09-override-animation.md)。
这些历史统计不是新增默认严格模式的成功率；严格模式有意拒绝其中的未知/拼错标签等输入。
