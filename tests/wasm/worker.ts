...
scope.onmessage = async ({ data: { fonts, output } }) => {
  ...
  try {
    ...
    require(defaultWarning.report.parse_mode === 'strict', 'New engine did not default to strict syntax')
    ...
    require(module._af_set_parse_mode(probe, 2) === 0, 'Invalid parse mode accepted')
    module._af_engine_destroy(probe)
    active.setMissingGlyphPolicy('error')
    active.setParseMode('compatible')
    expectError(() => active.process(missingInput), 'missing glyphs')
    require(active.process(input).subtitle === text, 'Parse mode switch changed output')
    active.setParseMode('strict')
    expectError(() => active.process(missingInput), 'missing glyphs')
