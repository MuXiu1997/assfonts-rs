...
- `--parse-mode strict|compatible`：默认 `strict` 拒绝未知/疑似拼错的标签，以及 `b/i/p/q` 的非完整 i32 整数、越界 `i/q` 参数；显式 `compatible` 使用 libass 兼容接受规则。两种模式都支持嵌套视觉动画，且不改写正文。此选项与缺字策略独立，JSON 报告新增 `parse_mode` 字段。详见 [解析模式](docs/parser-compatibility.md)。
...
- Aegisub 的 Project Garbage、Project、Extradata 段仅在字体分析时跳过，输出原样保留；段结构和事件字段诊断仍会报错，覆盖标签使用下述 libass 兼容规则。
...
- 兼容模式的覆盖标签按 libass 的有序前缀解释：`\border` 是描边，`\blu`/`\bklur` 会按 `\b` 改变字重；真正未知的标签消耗参数后忽略，不删除原文。默认严格模式会先拒绝这些未知/拼错标签。
- `\b2`、`\b4`、`\b20` 等非 0/1 且小于 100 的整数回退当前样式的 Bold，不统一视作粗体；100–900 的显式字重保持支持。
- `\N`、`\n`、`\h`、`\q`，绘图模式 `\p`；绘图坐标不作为字体字符，但非空绘图段仍收集当前字体、字重和斜体依赖。
- 常用位置、颜色、缩放、描边、淡入淡出、卡拉 OK 标签，以及不改变字体选择的 `\t`。
- 动画按 libass 的第一个右括号划分边界，支持视觉标签的嵌套 `\t`（最多 64 层）；无内部标签或缺右括号的恢复保留原文，内部字体/状态切换仍拒绝。
